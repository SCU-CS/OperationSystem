...
LPCWSTR CharToWchar(const char *str) {
    ...
    MultiByteToWideChar(CP_ACP, 0, str, sl + 1, wstr, sizeof(wstr) / sizeof(wstr[0]));
    wstr[sl] = 0;
    return wstr;
}
...
int main(int argc, char **argv) {
    ...
    while (inFile) {
        ...
        while (c != '\n' && !inFile.eof()) {
            ...
            // std::cout<<Thread_Info[n_Thread].thread_request[Thread_Info[n_Thread].n_request-1]<<' ';
            inFile.get(c);
        }
        // std::cout<<std::endl;
    ...
    for (int j = 0; j < (int)n_Thread; j++) {
        ...
        printf(" \nthread%2d    %c    %lf   ", Temp_serial, Temp_entity, Temp_delay);
    ...
    //使用的同步信号量命名；
    for (int j = 0; j < (int)n_Thread; j++) {
        std::string str= "semaphore_for_produce_";
        str+=std::to_string(j);
        puts(str.c_str());

        h_Semaphore[j + 1] = CreateSemaphore(NULL, 0, n_Thread, str.c_str());
    }
    ...
    for (int i = 0; i < (int)n_Thread; i++) {
        if (Thread_Info[i].entity == 'P')
            h_Thread[i] = CreateThread(
                NULL, 0, (LPTHREAD_START_ROUTINE)(Produce),
                &(Thread_Info[i]), 0, NULL);
        else
            h_Thread[i] = CreateThread(
                NULL, 0, (LPTHREAD_START_ROUTINE)(Consume),
                &(Thread_Info[i]), 0, NULL);
    }
    ...
    for (int i = 0; i < MAX_BUFFER_NUM; i++) {
        DeleteCriticalSection(&PC_Critical[i]);
    }
